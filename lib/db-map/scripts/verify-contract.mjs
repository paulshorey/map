...
const postgisExtensionResult = await client.query(`
  SELECT COUNT(*)::int AS count
  FROM pg_extension
  WHERE extname = 'postgis'
`);

if (getScalar(postgisExtensionResult.rows, "count") !== 1) {
  throw new Error("Missing expected extension: postgis");
}
...
const poiGeomIndexResult = await client.query(`
  SELECT COUNT(*)::int AS count
  FROM pg_indexes
  WHERE schemaname = 'public'
    AND indexname = 'pois_geom_gix'
`);

if (getScalar(poiGeomIndexResult.rows, "count") !== 1) {
  throw new Error("Missing expected index: pois_geom_gix");
}
