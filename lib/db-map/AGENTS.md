...
Per-source, resumable, idempotent stages. Category is always supplied by the developer via
`--category`; it is never inferred. Repeat `--category` to tag every record with multiple
categories — the first flag is the primary category (map marker, normalization profile,
denormalized `primary_category_id`), and the rest are additional categories on the POI.

```bash
pnpm --filter @lib/db-map ingest:run <file> --category <slug> [--category <slug> ...]
pnpm --filter @lib/db-map ingest:extract <source> <file> --category <slug> [--category <slug> ...] [--limit N] [--dry-run]
